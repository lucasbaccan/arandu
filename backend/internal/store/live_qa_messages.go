...
func (s *Store) CreateLiveQAMessage(ctx context.Context, m LiveQAMessage) (LiveQAMessage, error) {
	...
	_, err := s.db.ExecContext(ctx,
		`INSERT INTO live_qa_messages (id, event_id, participant_id, text, dismissed, created_at)
		 VALUES (?, ?, ?, ?, ?, ?)`,
		m.ID, m.EventID, participantID, m.Text, m.Dismissed, m.CreatedAt.Format(time.RFC3339),
	)
	if err != nil {
...
func (s *Store) ListLiveQAMessagesByEvent(ctx context.Context, eventID int64) ([]LiveQAMessage, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT id, event_id, COALESCE(participant_id, 0), text, dismissed, created_at
...
func scanLiveQAMessage(row scanner) (LiveQAMessage, error) {
	...
	err := row.Scan(&m.ID, &m.EventID, &m.ParticipantID, &m.Text, &m.Dismissed, &createdAt)
